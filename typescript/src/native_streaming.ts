import { RealtimeRawPointByVinsRequest, StreamEnvironment } from "@buf/compassiot_api.bufbuild_es/compassiot/platform/v1/streaming_pb"

import client from "./client"

// Insert your VINs here but ensure that they've been added to Compass
const VINS: string[] = []

const request = new RealtimeRawPointByVinsRequest({
    vins: VINS,
    streamEnv: StreamEnvironment.DEV,
})

for await (const response of client.realtimeRawPointByVins(request)) {
    console.log(response.toJsonString({ prettySpaces: 2 }))
}
